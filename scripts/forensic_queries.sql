-- ================================================================================
-- FORENSIC STOCK VALUE ANALYSIS
-- Run these queries in PostgreSQL to identify discrepancies
-- ================================================================================

-- ================================================================================
-- 1. SOURCE DATABASE TOTALS
-- ================================================================================
\echo '================================================================================'
...
SELECT 
    ...
    ROUND(SUM(BALANCE)::numeric, 2) as total_quantity,
    ROUND(SUM(PRICE_EX_VAT * BALANCE)::numeric, 2) as value_ex_vat,
    ROUND(SUM(SELLING_PRICE * BALANCE)::numeric, 2) as value_inc_vat,
    ROUND(SUM((SELLING_PRICE - PRICE_EX_VAT) * BALANCE)::numeric, 2) as total_vat_amount
FROM source_stock
WHERE BALANCE > 0;
...
SELECT 
    ...
    COUNT(*) as item_count,
    ROUND(SUM(BALANCE)::numeric, 2) as total_qty,
    ROUND(SUM(PRICE_EX_VAT * BALANCE)::numeric, 2) as value_ex_vat,
    ROUND(SUM(SELLING_PRICE * BALANCE)::numeric, 2) as value_inc_vat,
...
SELECT 
    ...
    ROUND(COALESCE(s.src_qty, 0)::numeric, 2) as source_qty,
    ROUND(COALESCE(t.tgt_qty, 0)::numeric, 2) as target_qty,
...
-- ================================================================================
-- 4. MISSING IN TARGET (Top 20 by value)
-- ================================================================================
\echo ''
\echo '================================================================================'
\echo 'ITEM-LEVEL DISCREPANCIES: Missing in Target (Top 20 by value)'
\echo '================================================================================'
\echo ''
...
SELECT 
    ...
    ROUND(s.BALANCE::numeric, 2) as source_qty,
...
-- ================================================================================
-- 5. MISSING IN SOURCE (Top 20 by value)
-- ================================================================================
\echo ''
\echo '================================================================================'
\echo 'ITEM-LEVEL DISCREPANCIES: Missing in Source (Top 20 by value)'
\echo '================================================================================'
\echo ''
...
SELECT 
    ...
    ROUND(t.current_stock::numeric, 2) as target_qty,
    ROUND((
        CASE 
...
-- ================================================================================
-- 6. QUANTITY MISMATCHES (Top 20 by difference)
-- ================================================================================
\echo ''
\echo '================================================================================'
\echo 'ITEM-LEVEL DISCREPANCIES: Quantity Mismatches (Top 20)'
\echo '================================================================================'
\echo ''
...
-- ================================================================================
-- 7. VAT RATE DIFFERENCES (Top 20 by value impact)
-- ================================================================================
\echo ''
\echo '================================================================================'
\echo 'ITEM-LEVEL DISCREPANCIES: VAT Rate Differences (Top 20 by value impact)'
\echo '================================================================================'
\echo ''
...
SELECT 
    s.STOCK_CODE,
    SUBSTRING(s.DESCRIPTION, 1, 30) as description,
    ROUND(s.BALANCE::numeric, 2) as qty,
...
\echo 'Dashboard should show:     (target_total_inc_vat from above after VAT fix applied)'
\echo ''
