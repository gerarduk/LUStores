...
try {
    ...
    Write-Host "Please ensure your domain is pointing to this server's IP address" -ForegroundColor Yellow
    exit 1
}
...
if ((Test-Path "certbot\conf\live\$Domain") -and (-not $Force)) {
    ...
    Write-Host "To renew certificates: docker-compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml exec certbot certbot renew" -ForegroundColor Yellow
    Write-Host "To force regeneration: .\setup-ssl.ps1 -Force" -ForegroundColor Yellow
    exit 0
}
...
if (Test-Path "certbot\conf\live\$Domain") {
    ...
    Write-Host "1. Ensure your domain $Domain points to this server's IP"
