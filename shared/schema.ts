...
export const saleItems = pgTable("sale_items", {
  ...
  vatIncluded: boolean("vat_included").notNull(), // Snapshot of whether price included VAT at time of sale
  ...
  saleItemIdx: index("idx_sale_items_sale_item").on(table.saleId, table.itemId),
}));
...
export const orders = pgTable("orders", {
  ...
  vatRate: decimal("vat_rate", { precision: 5, scale: 4 }).default("0.2000"), // VAT rate for the order
  vatIncluded: boolean("vat_included").notNull().default(true), // Whether order prices include VAT
  updateInventoryValues: boolean("update_inventory_values").notNull().default(false), // Whether to update item prices during receiving
  ...
  supplierStatusIdx: index("idx_orders_supplier_status").on(table.supplierId, table.status),
}));
...
export const orderItems = pgTable("order_items", {
  ...
  vatRate: decimal("vat_rate", { precision: 5, scale: 4 }).notNull().default("0.2000"), // VAT rate for this item
  vatAmount: decimal("vat_amount", { precision: 10, scale: 2 }).notNull().default("0.00"), // VAT amount for this item
  received: boolean("received").notNull().default(false),
  receivedQuantity: decimal("received_quantity", { precision: 10, scale: 2 }),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  itemIdx: index("idx_order_items_item_id").on(table.itemId),
}));
...
export const insertOrderSchema = z.object({
  ...
  updateInventoryValues: z.boolean().optional(),
  createdBy: z.string(),
});
...
export const insertOrderItemSchema = z.object({
  ...
  totalCost: z.string(),
  vatRate: z.union([z.number(), z.string()]).transform(val => parseFloat(val.toString())).optional(),
  vatAmount: z.union([z.number(), z.string()]).transform(val => parseFloat(val.toString())).optional(),
...
export type ChargeCodeExclusion = typeof chargeCodeExclusions.$inferSelect;
export type InsertChargeCodeExclusion = z.infer<typeof insertChargeCodeExclusionSchema>;
export type ChargeCodeAuthorizedUser = typeof chargeCodeAuthorizedUsers.$inferSelect;
export type InsertChargeCodeAuthorizedUser = typeof chargeCodeAuthorizedUsers.$inferInsert;
...
export type InsertPermissionDefinition = typeof permissionDefinitions.$inferInsert;
export type SystemTest = typeof systemTests.$inferSelect;
export type ChargeCodeAssignment = typeof chargeCodeAssignments.$inferSelect;
export type InsertChargeCodeAssignment = typeof chargeCodeAssignments.$inferInsert;
 ...
 */
export function formatStockDisplay(value: string | number): string {
  const num = typeof value === 'string' ? parseFloat(value) : value;
 ...
 */
export function parseStockQuantity(value: string | number): number | null {
  const num = typeof value === 'string' ? parseFloat(value) : value;
